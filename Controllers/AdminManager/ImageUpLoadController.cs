...
{
    private readonly bobateashopContext _context;
    private readonly ILogger<ImageUploadController> _logger;

    public ImageUploadController(bobateashopContext context, ILogger<ImageUploadController> logger)
    {
        _context = context;
        _logger = logger;
    }
    ...
    public async Task<IActionResult> UploadImages([FromForm] IFormFileCollection files)
    {
        ...
        // Sử dụng Path.Combine để đảm bảo đường dẫn được xử lý đúng trên Windows
        var uploadPath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "img", "boba-img");
        ...
        foreach (var file in files)
        {
            if (file.Length > 0)
            {
                try
                {
                    ...
                    var fileUrl = $"/img/boba-img/{uniqueFileName}";
    ...
    public IActionResult DeleteImage(string id)
    {
        ...
        try
        {
            ...
            var imagePath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "img", "boba-img", id);
